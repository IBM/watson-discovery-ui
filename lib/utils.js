  ...
  for (let index=0; index<data.results.length; index++) {
    ...
    dataItem.highlight.text.forEach(function(element) {
      ...
      let newResult = {
        ...
        textBlurb: element,
        textFull: element,
        date: dataItem.extracted_metadata.filename,
 ...
 */
// function formatData2(rawData, passages, filterString) {
//   let formattedData = {};
//   let newResults = [];
//   let data = rawData.rawResponse.result;
...
//     // only keep the data we show in the UI.
//     let dataItem = data.results[index];
//     console.log('FILENAME: ' + dataItem.extracted_metadata.filename);
...
//     let initialTextLen = (dataItem.text.length > 200)? 200 : Math.ceil(dataItem.text.length / 2);
...
//       addResult = false;
//       for (let i=0; i<passages.results.length; i++) {
//         let res = passages.results[i];
