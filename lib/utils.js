  ...
  for (let index=0; index<data.results.length; index++) {
    ...
    if (returnPassages) {
      ...
      if (typeof filterString !== 'undefined' && filterString.length > 1) {
        ...
        findHighlightedText(dataItem.enriched_text[0].entities, dataItem, filterString, newResult, 'entities');
        findHighlightedText(dataItem.enriched_text[0].keywords, dataItem, filterString, newResult, 'keywords');
      }
    }
...
function findHighlightedText(collection, dataItem, filterString, newResult, itemType) {
  collection.forEach(function(item) {
    if (typeof item.mentions !== 'undefined' && item.mentions.length > 0) {
      item.mentions.forEach(function(mention) {
        ...
        var highlightText = 'enriched_text.' + itemType + '.text::"' +
        ...
        if ((filterString.indexOf(highlightText) >= 0) ||
            (filterString.indexOf(item.type) >= 0)) {
          newResult.highlight.showHighlight = true;
