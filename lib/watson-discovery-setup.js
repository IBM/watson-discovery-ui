...
/**
 * Get the specified Discovery configuration.
  ...
  return new Promise((resolve, reject) => {
    this.discoveryClient.getConfigurations(params, (err, data) => {
      if (err) {
        ...
        var default_config_id = '';
        const configs = data.configurations;
        for (let i = 0, size = configs.length; i < size; i++) {
          const config = configs[i];
          if (config.name === 'Default Configuration') {
            default_config_id = config.configuration_id;
          } else if (config.name === params.config_name) {
            params.configuration_id = config.configuration_id;
            return resolve(params);
          }
        }
        // use default config if we can't find the specified config
        if (default_config_id != '') {
          console.log("Unable to find config name: " + params.config_name + '. Using Default.');
          params.configuration_id = default_config_id;
          return resolve(params);
        }

        return reject(new Error(
          'Failed to get find Discovery configuration: ' + params.config_name));
      }
    });
  });
};
